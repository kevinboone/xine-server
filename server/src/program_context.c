...
ProgramContext *program_context_create (void)
  ...
  self->props = props;
  props_put_integer (props, "log-level", MYLOG_INFO);
...
BOOL program_context_parse_command_line (ProgramContext *self, 
  ...
  static struct option long_options[] =
    {
      ...
      {"list-drivers", no_argument, NULL, 0},
      {0, 0, 0, 0}
    };
   ...
   while (ret)
     {
     int option_index = 0;
     opt = getopt_long (argc, argv, "h:vl:p:d:c:",
     ...
     switch (opt)
       ...
       case 0:
         if (strcmp (long_options[option_index].name, "help") == 0)
           program_context_put_boolean (self, "show-usage", TRUE);
             ...
             "list-drivers") == 0)
           program_context_put_boolean (self, "list-drivers", TRUE);
         ...
         else if (strcmp (long_options[option_index].name, "driver") == 0)
           program_context_put (self, "driver", optarg); 
           ...
           program_context_put (self, "config", optarg); 
         else
           exit (-1);
         break;
       case '?': 
       ...
       case 'p': program_context_put_integer (self, "port", 
           atoi (optarg)); break;
       case 'd': program_context_put (self, "driver", optarg); break;
       case 'h': program_context_put (self, "host", optarg); break;
       case 'c': program_context_put (self, "config", optarg); break;
       default:
         ret = FALSE; 
       }
    }
  ...
  if (program_context_get_boolean (self, "debug", FALSE))
    {
    self->is_debug = TRUE;
    }
...
/*==========================================================================
  program_context_is_debug
==========================================================================*/
BOOL program_context_is_debug (const ProgramContext *self)
  {
