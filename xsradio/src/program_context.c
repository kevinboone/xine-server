...
BOOL program_context_parse_command_line (ProgramContext *self, 
  ...
  static struct option long_options[] =
   ...
   while (ret)
     {
     int option_index = 0;
     opt = getopt_long (argc, argv, "?h:vl:p:s:c:",
     ...
     switch (opt)
       ...
       case 0:
         ...
         else if (strcmp (long_options[option_index].name, "streams") == 0)
           program_context_put (self, "streams", optarg); 
       ...
       case 's': program_context_put (self, "streams", optarg); break;
       case 'c': program_context_put (self, "colour", optarg); break;
       default:
         ret = FALSE; 
       }
    }
